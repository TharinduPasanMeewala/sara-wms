...
const API = 'http://localhost:3000/api/inventorys'

export default function InventoryPage() {
  const [inventorys, setInventorys] = useState([])
  const [loading, setLoading] = useState(true)
  const [showForm, setShowForm] = useState(false)
  const [form, setForm] = useState({inventory_id: '', tenant_id: '', location_id: '', sku_id: '', lot_serial_id: ''})
  ...
  async function load() {
    ...
    try {
      ...
      if (res.ok) setInventorys(await res.json())
  ...
  async function save() {
    ...
    setShowForm(false); setEditing(null); setForm({inventory_id: '', tenant_id: '', location_id: '', sku_id: '', lot_serial_id: ''}); load()
  }
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage Inventory records</p>
        </div>
        <button onClick={() => { setShowForm(true); setEditing(null); setForm({inventory_id: '', tenant_id: '', location_id: '', sku_id: '', lot_serial_id: ''}) }} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">
          + Add Inventory
        </button>
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md">
            <h2 className="text-lg font-bold text-gray-900 mb-4">{editing ? 'Edit' : 'New'} Inventory</h2>
            <div className="space-y-4">
              
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">inventory_id</label>
              <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={form.inventory_id || ''} onChange={e => setForm(p => ({...p, inventory_id: e.target.value}))} />
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">location_id</label>
              <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={form.location_id || ''} onChange={e => setForm(p => ({...p, location_id: e.target.value}))} />
            </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">inventory_id</th>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">tenant_id</th>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">location_id</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {inventorys.map(item => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 text-sm text-gray-700">{String(item.inventory_id ?? '')}</td>
                  <td className="px-4 py-3 text-sm text-gray-700">{String(item.tenant_id ?? '')}</td>
                  <td className="px-4 py-3 text-sm text-gray-700">{String(item.location_id ?? '')}</td>
